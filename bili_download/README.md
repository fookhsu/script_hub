...
└── test/
    ...
    ├── bili_download.test.js   # 13 个集成测试用例（含 1 个回归修复用例）
    └── node_modules/
```
...
| 单P下载 | 点击弹框内某个分P标题，直接 `window.open` 该P的播放直链 | 同上 |
| 批量下载 | 勾选多P后通过 **aria2 JSON-RPC(WebSocket)** 推送到 Motrix / AriaNgGUI，支持自定义 RPC 地址/Token/保存路径，设置自动记忆 | 同上 |
...
| 已修复缺陷 | — | 服务器导航菜单 `toast.show` 未定义报错（原版真实 Bug）；dialog 重复关闭/字符串参数异常；toast 动画不触发时元素不清理；aria2 单选框用 `checked` 属性选中、RPC 预填；aria2 连接失败/断开时 Promise 正确 reject；简介转链接加标记防 MutationObserver 无限循环 |
...
npm test                         # ① 默认：对 优化版（部署版）跑测试，13/13 全绿
npm run test:original            # ② 对 原版 bili_download.js 跑基线（12 通过 + 1 已知失败提示，退出码仍为 0）
...
# 原版基线
KNOWN_FAIL_SUBSTR=服务器导航设置 SCRIPT_PATH=./bili_download.js node test/bili_download.test.js
```

`KNOWN_FAIL_SUBSTR`：把名称中匹配该关键词的失败用例标记为“已知失败（预期）”而不影响退出码——原版基线里唯一失败的就是**已修复的原版 Bug（服务器导航菜单 `toast.show` 未定义）**，正常跑原版时应显示：

```
结果: 12/13 通过（另 1 个为已知预期失败）
```
...
| 目标脚本 | 结果 | 说明 |
| --- | --- | --- |
| `bili_download.optimized.js`（默认/部署版） | **13/13** 通过 | 无 jQuery 环境下同样全绿 |
| `bili_download.js`（原版基线） | 12/13 通过 + 1 已知失败 | 失败项即原版真实 Bug：菜单里 `toast.show` 引用了未定义变量 |

### 用例清单（test/bili_download.test.js）

1. 通用页面加载无报错，注册“功能开关”菜单
2. 视频页渲染工具栏与多P弹框（mock view 接口返回 3P）
3. 批量下载选中 2P → 校验 playurl 请求次数与 2 个 aria2 WebSocket 连接、保存下载配置
4. 搜索页已看视频显示“已看”角标（预置浏览记录）
5. 视频页浏览后 BV 写入缓存
6. “功能开关”弹框开关选项并保存、toast 提示
7. 简介中的外部网址转成 `<a target="_blank">`，且不重复生成/不死循环
8. 单P下载点击后 `window.open` 播放直链
9. 云厂商页渲染服务器导航容器、注入 API 返回 HTML、锚点追加 track 参数并清理 data-spm
10. 合集(ugc_season)分集数据渲染成可下载列表（5 个分集片段）
11. （回归）弹框默认真正选中 Motrix 单选框
12. （回归）切换 AriaNgGUI 自动带出 ws://localhost:6800 RPC 地址
13. （回归，对应原版 Bug）服务器导航设置菜单保存后正常弹提示
